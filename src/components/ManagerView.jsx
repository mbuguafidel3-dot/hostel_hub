...
const ManagerView = ({ hostels, bookings, viewings, onUpdate }) => {
  ...
  const [newHostel, setNewHostel] = useState({ name: '', location: '', total_units: '' });
    ...
    <>
      <header className="dashboard-header">
        ...
        <button 
          ...
          style={{ display: 'flex', gap: '8px', width: 'auto', padding: '12px 24px' }} 
...
<Tabs 
        ...
        <section>
          ...
          {hostels.length > 0 ? (
            ...
            <div className="card" style={{ textAlign: 'center', padding: '60px' }}>
              <Building2 size={48} color="var(--primary-light)" style={{ margin: '0 auto 20px' }} />
      ...
      {activeTab === 'applications' && (
        <section>
          ...
          <BookingList bookings={bookings.filter(b => b.status === 'pending')} onUpdate={onUpdate} />
        </section>
      )}
      ...
      {activeTab === 'placements' && (
        <section>
          ...
          <BookingList bookings={bookings.filter(b => b.status === 'assigned')} onUpdate={onUpdate} />
        </section>
      )}
          ...
          <div className="auth-card modal-content">
            ...
            <form onSubmit={handleAddHostel}>
              <div className="form-group">
                ...
                <input 
                  type="text" className="auth-input" style={{ paddingLeft: '15px' }} required
              ...
              <div className="form-group">
                ...
                <div className="input-with-icon">
                  ...
                  <input 
                    ...
                    value={newHostel.location} onChange={e => setNewHostel({...newHostel, location: e.target.value})}
                  />
                </div>
              </div>
              <div className="form-group">
                ...
                <input 
                  type="number" className="auth-input" style={{ paddingLeft: '15px' }} required
                  value={newHostel.total_units} onChange={e => setNewHostel({...newHostel, total_units: e.target.value})}
                />
              </div>
              <div style={{ display: 'flex', gap: '15px', marginTop: '30px' }}>
                <button type="submit" className="auth-button">Add Hostel</button>
                <button type="button" className="auth-button" style={{ background: 'var(--bg-main)', color: 'var(--text-main)' }} onClick={() => setShowAddHostel(false)}>Cancel</button>
              </div>
            </form>
          </div>
        </div>
