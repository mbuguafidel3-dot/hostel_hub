...
bookings.patch('/:id/assign', async (c) => {
  ...
  try {
    ...
    const targetBooking = db.prepare(`
    ...
    const otherAssigned = db.prepare(`
      SELECT id FROM bookings 
      WHERE student_id = ? AND status = 'assigned' AND notice_given = 0 AND id != ?
    `).get(targetBooking.student_id, id);

    if (otherAssigned) {
      return c.json({ error: 'Student already has an active assignment in another hostel' }, 400);
    }

    db.prepare('UPDATE bookings SET unit_number = ?, status = ? WHERE id = ?')
...
bookings.patch('/:id/notice', async (c) => {
  ...
  try {
    const booking = db.prepare('SELECT id FROM bookings WHERE id = ? AND student_id = ? AND status = ?')
    ...
    db.prepare("UPDATE bookings SET notice_given = 1, notice_date = CURRENT_TIMESTAMP WHERE id = ?")
      .run(id);
...
bookings.patch('/:id/leave', async (c) => {
  ...
  try {
    ...
    const archiveTransaction = db.transaction(() => {
      const fullBooking = db.prepare('SELECT * FROM bookings WHERE id = ?').get(id);
...
bookings.post('/', async (c) => {
  ...
  try {
    ...
    const viewing = db.prepare("SELECT id FROM viewings WHERE hostel_id = ? AND student_id = ? AND status = 'completed'").get(hostel_id, studentId);
    const inHistory = db.prepare("SELECT id FROM booking_history WHERE hostel_id = ? AND student_id = ? LIMIT 1").get(hostel_id, studentId);

    if (!viewing && !inHistory) {
      return c.json({ error: 'You must complete a viewing or have a past stay before requesting consideration' }, 403);
    }

    // 2. Check if student is already assigned somewhere without notice
    const currentAssigned = db.prepare(`
      SELECT id FROM bookings 
      WHERE student_id = ? AND status = 'assigned' AND notice_given = 0
    `).get(studentId);

    if (currentAssigned) {
      return c.json({ error: 'You must give leave notice at your current hostel before applying for a new one' }, 400);
    }

    // 3. Check if already applied
    const existing = db.prepare("SELECT id FROM bookings WHERE hostel_id = ? AND student_id = ? AND status != 'completed'").get(hostel_id, studentId);
    if (existing) return c.json({ error: 'You have an active application or assignment for this hostel' }, 400);

    const info = db.prepare(
      'INSERT INTO bookings (hostel_id, student_id, status) VALUES (?, ?, ?)'
    ).run(hostel_id, studentId, 'pending');

    return c.json({ message: 'Request for consideration sent successfully', id: info.lastInsertRowid }, 201);
  } catch (err) {
