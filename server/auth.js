...
auth.post("/register", async (c) => {
  const { fullname, password, email, role, student_number } =
    await c.req.json();
  try {
    ...
    console.log(
      `[Register] User created: ${email} (${role}) ${student_number ? "[" + student_number + "]" : ""}`,
    );
    return c.json(
      { message: "User registered successfully", userId: info.lastInsertRowid },
      201,
    );
  } catch (err) {
    if (err.message.includes("UNIQUE constraint failed")) {
      ...
      if (err.message.includes("student_number")) {
        console.warn(
          `[Register] Student number already exists: ${student_number}`,
        );
